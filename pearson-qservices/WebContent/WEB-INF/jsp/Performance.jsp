...
<c:if test="${!empty loginType}">
			<c:if test="${loginType=='VISITOR'}">
					...
					<form
						...
						<input type="hidden" name="service" value="PERFORMANCE" /> 
						<input type="hidden" name="serviceID" value="PT10" /> 
		...
		<div class="span-24">
		<H1 style="color:#900000">Performance Testing</H1>
			
<h2 class="myNewStyle" style="color:#3399FF">  Key Features Offerings </h2>

	
	<p > Robust & Tool agnostic Performance testing Framework to convert the business requirements into measurable performance requirements, to determine required test volume and to prepare estimation & test schedule.. <br>
Develop, review and validate test scripts and create required workload scenarios. <br>
Posses unparalleled skillset on multiple tools such as HP performance center, Jmeter Load UI & SOAP UI. <br>
High Quality procedures and guidelines for performance testing using sophisticated templates. <br>
Performance testing  experience on various other tools such as Web Based Apps, Web services, REST API services, ESB, DataHub, eCommerce platform, Identity management, CRM and ERP applications. </p>


<h2 style="color:#3399FF">Tools & Accelerators</h2>
	<p>HP Performance center is a tool which can create scenarios, monitors statistics while running the performance scripts. It can host multiple projects and accounts for multiple users. <br>
Performance testing Framework defines standards and activities for each phase of performance testing which helps in addressing client requirements effectively. <br>
JMeter is an open-source performance testing tool that can discover maximum number of concurrent users that a website can handle and provides provides a variety of graphical analyses of performance reports. <br>
JMeter and Jenkins integration for running Performance test scripts. <br>
Sophisticated Performance testing lab across different regions like US, UK & China. <br>
SOAP UI is a tool from Smart bear which helps in web services functional testing, data driven testing and web services load testing. <br>
New Relic is used for monitoring web and mobile applications. It provides response times break up across the components of application and performs detailed error analysis. <br>
SAAVIS is a monitoring tool for web applications.<br></p>

<h2 style="color:#3399FF">Where this service would be benefiting</h2>
	<p> Performance testing can be executed to assess end user response time and server level scalability. <br>
Performance testing can be executed to identify the impact of a particular system after the increase in the volume of user community & data. <br>
Performance testing is used to assess production readiness by evaluating against performance criteria and find the source of performance problems. <br>
To compare performance characteristics of multiple systems or system configurations. <br>
It supports system tuning and helps in determining throughput levels.
		</p>
...
<h2 style="color:#3399FF">Key Highlights from Projects Executed</h2>
	<p>1. Identified major issues with Simplified while testing SSO and recommended vendor to provide fixes.<br>
2. Identified major issues with elasticpath APIs during performance testing.<br>
3. Identified issues with Composite tool(Data Hub) during performance testing of ICOM 1.3 and ensured that Composite provided fixes that are necessary to achieve the SLAs defined.<br>
4. For the first time, provided solution successfully to conduct batch/data performance testing<br>
5. For the first time, provided solution successfully to conduct API performance testing. This helped the client to identify the performance issues at API level itself and make the necessary fixes to the application.<br>
6. For first time, implemented new tools like New Relic & SAAVIS for monitoring server metrics, SQL developer to fetch batch processing elapsed times and analyzed elastic path server logs.
</p>
